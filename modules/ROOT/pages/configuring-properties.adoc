...
In case you want to use the Properties Placeholder prefix (`${`) as literals of your configuration property, you can escape the value. For example: 
[source,yaml,linenums]
----
some:
	property: "The properties' placeholder prefix is: \\${"
----
...
Also, a property can derive its value from other properties that are higher in the hierarchy. Therefore, an application property can derive its value from environment, system, and/or deployment properties. A system property can derive its value from a deployment property, and so on. For example, if there is a system property named `env`, an application configuration property can have the value `file.$&#123;env&#125;.xml`.
